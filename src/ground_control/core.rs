...
async fn reader_task(read_half: tokio::io::ReadHalf<TcpStream>, tx_to_core: mpsc::Sender<LinkMsg>) {
    ...
    loop {
        match lines.next_line().await {
            ...
            Ok(None) => { warn!("GCS peer disconnected"); break; }
            Err(e) => { error!("GCS read error: {}", e); break; }
        }
    }
}
...
async fn writer_task(mut write_half: tokio::io::WriteHalf<TcpStream>, mut rx_from_core: mpsc::Receiver<LinkMsg>) {
    while let Some(msg) = rx_from_core.recv().await {
        let mut s = match serde_json::to_string(&msg) {
            Ok(v) => v,
            Err(e) => { warn!("GCS serialize failed: {}", e); continue; }
        };
        s.push('\n');
...
pub async fn start_server(bind: &str) -> Result<TcpLink, String> {
    let listener = TcpListener::bind(bind).await.map_err(|e| format!("bind failed: {e}"))?;
    info!("GCS listening on {}", bind);

    let (stream, addr) = listener.accept().await.map_err(|e| format!("accept failed: {e}"))?;
